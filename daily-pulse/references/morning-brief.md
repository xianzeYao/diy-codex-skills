...
8. Read Topics config and build a compact topic brief.
...
When rewriting or refining an existing morning report, preserve a `Topic Intelligence` section. If no topic creates an action, still include each checked topic with `monitor` or `ignore` so the user can see that topic tracking ran.
