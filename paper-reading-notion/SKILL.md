...
- `现况与动机` is a research-gap diagnosis, not a paraphrase of the paper's narrative. Cross-check the introduction and related work against the cited papers when needed and the most relevant existing PaperReading notes. Explain: the field's competing routes; what each route solves and assumes; the gap the authors explicitly identify; and material blind spots, confounds, missing comparisons, or untested assumptions that the paper does not identify. Separate `【作者指出的缺口】`, `【交叉检验后的缺口】`, and `【论文未覆盖的盲区】` when useful. Then distinguish the authors' claimed contribution from the contribution actually supported by the evidence. Use plain language before technical terminology, and link only the 2-4 existing PaperReading notes that materially clarify the relationship (same problem, alternative route, complementary mechanism, or conflicting evidence).
...
- `实验` must cover each important experiment, including result-table screenshots and key figures. For every experiment: first state the hypothesis or alternative explanation the authors are trying to test or rule out; then explain setting, baselines, metric, main result, and author interpretation; finally state what the evidence proves, what remains unproven, and the concrete missing experiment/control. Prefer screenshots/crops of paper tables over recreated Notion tables.
...
- Each experiment subsection should be detailed enough to stand alone: state the hypothesis or competing explanation being tested, why this experiment is necessary, evaluation setting/protocol, compared variants and what each variant means, metrics, main numbers, author interpretation, your analysis, what the result does and does not prove, and the precise missing control/experiment. Do not collapse multiple ablations, scaling trends, and benchmarks into one short result dump.
- `思考` must consider limitations the authors state themselves, then add the user's/agent's own critique only after separating it from the author-stated limitation. `🔬` points must be further verifiable ideas: name the mechanism hypothesis, the intervention or comparison, the observable metric, and the result pattern that would support or falsify the idea. Do not use it for generic future-work wishes.

## Create Mode Workflow

Use create mode by default when the user says "写 Notion", "新建", "创建页面", or asks to write a new paper note from a paper source. Before creating, establish a page identity from the resolved PDF hash when available, DOI/arXiv ID, and normalized title. Search direct children of `实验室工作 / PaperReading` using that identity. If a canonical page exists, update that page by default, even when the user says “再做一遍”; create a second page only when the user explicitly asks for a separate version. Report any ambiguous near-duplicate before writing, never silently create another copy.
...
6. Build the Notion page:
   ...
   - `现况与动机`: start from the research problem rather than the paper's section order. Map prior routes from introduction/related work and the closest existing PaperReading notes; identify the authors' stated gap and independently diagnose important blind spots that the paper omits. Explain the gap in plain language, then separate `作者声称的贡献`, `证据实际支持的贡献`, and what remains unresolved before introducing the method.
   ...
   - `实验`: question/hypothesis titles, beginning with why the authors need this experiment and which rival explanation it can rule out; then baseline descriptions, evaluation set/protocol, screenshots of main result tables/figures, main results, important ablations, author interpretation, what the result proves and does not prove, and the exact missing control. Include enough factual setup before critique. If a method, baseline, variant, dataset, or action representation appears only in the experiment table and has not been explained clearly, explain it locally before discussing its score. Do not use table/figure labels as subsection headings. Do not recreate result tables as Notion tables unless screenshots are unavailable and the limitation is reported.
   - `思考`: exactly three compact Notion callouts in the template style, not plain quote blocks: reusable insight (`💡`), questions/risks/limitations (`❓`), and further verifiable research ideas (`🔬`). Use gray background. Inside each callout, write numbered points (`1.`, `2.`, `3.` as needed). Include author-stated limitations first when available, then add Debate/Revised Takeaway critique for claims that should be weakened and alternative explanations. Every `🔬` point must specify a mechanism hypothesis, intervention/comparison, measurable outcome, and supporting versus falsifying result pattern. Keep each point tied to a mechanism, evidence gap, or experiment design.
...
4. **Motivation**: `现况与动机` diagnoses the research gap rather than reproducing the paper narrative. It cross-checks introduction/related work with the most relevant existing notes when available; distinguishes author-stated gaps from material blind spots the paper did not mention; explains the relationships to only relevant prior notes; and separates claimed contribution from evidence-supported contribution.
...
8. **Experiments**: each important experiment includes the hypothesis or alternative explanation being tested, why the test is needed, setting, baselines/variants with local explanations, metrics, result asset, author's analysis, the agent's analysis of what is and is not proven, and the exact missing experiment/control.
